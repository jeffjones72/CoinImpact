...
class Action_new_model extends CI_Model {
    ...
    public function get_combatant($place_id, $rarity_level, $player_id) {
        ...
        $this->db->select('combatant');
        $this->db->from('combatants');
        $this->db->join('combatant_sections','combatants.section_id = combat_sections.id');
        $this->db->join('places_allowed_combatant_sections','combatants.section_id = places_allowed_combatant_section.combatantsection_id');
        $this->db->where('places_allowed_combatant_sections.place_id'.$place_id);
        $this->db->where('rarity_id <= ' . $rarity_level);
        $this->db->order_by('id', 'random');
        $this->db->limit(1);
        $query = $this->db->get();
        echo 'action_new_model:get_combatant';
