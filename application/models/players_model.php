...
class Players_model extends CI_Model {
    ...
    function get_player_info($account_id) {
		...
		where p.account_id = ' . $account_id; //TODO | CRITICAL: MAKE SURE THIS IS NOT AN SQL INJECTION
    ...
    public function seen_base_intro($player_id){
        $this->db->set('base_intro', 1, FALSE);
        $this->db->where('id', $player_id);
        $this->db->update('players');
    }
