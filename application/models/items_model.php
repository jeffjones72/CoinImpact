...
class Items_model extends CI_Model
{
    ...
    {
        $this->db->select('i.*, i.id as item_id, quality, i.name as item_name, i.description as item_description, r.name as rarity_name ,
            count(item_id) as pieces
            ');
        $this->db->from('items i');
        $this->db->join('rarities r', 'i.rarity_id = r.id');

        if ($player_id != 0) {
            $this->db->join('player_items pi', 'pi.item_id = i.id and pi.player_id = ' . $player_id, 'LEFT OUTER');
            $this->db->group_by('item_id, quality');
        }
        ...
        if ($query->num_rows > 0) {
            ...
            /*
             * CODE: CI:B0201 calculates the number of items the player has as pieces
             */
            $this->db->select('pi.quality, pi.durability, pi.id as player_item_id, count(i.id) as pieces ,i.id, i.name, i.description, i.price, i.premium_price, i.value, i.section_id,i.has_quality, i.weight, coalesce(pi.mod_atk,0) + i.attack as attack, coalesce(pi.mod_def,0) + i.defense as defense, i.energy, i.stamina, i.health, i.strike, i.strike_boost, i.damage_boost, i.luck, i.dodge, i.capacity, i.modifiers_limit,i.rarity_id, pi.stored', FALSE);
            $this->db->from('player_items pi');
            $this->db->join('items i', 'pi.item_id = i.id');
            $this->db->where('pi.slot_id is null');
            $this->db->where('coalesce(pi.stored,0)',0,false);
            $this->db->where('pi.player_id', $player_id);
            $this->db->group_by("i.id, pi.quality");
            $query = $this->db->get();
            ...
            // Add query for stored items
            $this->db->select('pi.quality, pi.durability, pi.id as player_item_id, count(i.id) as pieces ,i.id, i.name, i.description, i.price, i.premium_price, i.value, i.section_id,i.has_quality, i.weight, coalesce(pi.mod_atk,0) + i.attack as attack, coalesce(pi.mod_def,0) + i.defense as defense, i.energy, i.stamina, i.health, i.strike, i.strike_boost, i.damage_boost, i.luck, i.dodge, i.capacity, i.modifiers_limit,i.rarity_id, pi.stored', FALSE);
            $this->db->from('player_items pi');
            $this->db->join('items i', 'pi.item_id = i.id');
            $this->db->where('pi.player_id', $player_id);
            $this->db->where('pi.stored', 1);
            $query = $this->db->get();
    ...
    //ticket #58 get items with slots
    function get_slot_items($player_id){

        $this->db->select('*', FALSE);
        $this->db->from('player_items ')->where("player_id = {$player_id} and slot_id is not null");

        $data = $this->db->get()->result();
        foreach ($data as $result){
            $result->item=$this->db->get_where("items", array("id" => $result->item_id))->row();
        }
        return $data;
    }
    public function get_players_items($player_id){

        $this->db->select('i.*, pi.*,   i.id as id_item, i.name as item_name, i.description as item_description, r.name as rarity_name ,
            count(item_id) as pieces
            ');
        $this->db->from('items i');
        $this->db->join('rarities r', 'i.rarity_id = r.id');
        ...
        if ($player_id != 0) {
            $this->db->join('player_items pi', 'pi.item_id = i.id ', 'LEFT');
            $this->db->where("pi.player_id = {$player_id}");
        }
    ...
    function insert($player_id, $item_id)
    {
        $date = date("Y-m-d H:i:s", time());

        $player = newPlayer();
        $player->tryAdd(new Item($item_id));
        /*
         * $arr = array(
         * 'player_id' => $player_id,
         * 'item_id' => $item_id,
         ...
         * );
         *
         * $this->db->insert('player_items', $arr);
         */
    }

    function insert_first($player_id, $item_id, $slot_id)
    {
        $player = newPlayer($player_id);
        $player->tryAdd(new Item($item_id), $slot_id);
        /*
         * $date = date("Y-m-d H:i:s", time());
         * if ($slot_id != 0) {
         * $arr = array(
