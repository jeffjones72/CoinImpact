...
<div id="drop_modal" class="modal">
	...
	<form method="post" style="width: 150px; float: left; margin-left: 20px;">
...
// 	die();
	...
	?>
	<input type="hidden" name="player_id" value="<?php echo $player->id;?>" id="input_id">
	   <select name="squad_members"  class="selectorSquad" >
	...
	<div id="playerStats">
		<div class="bulletBg">
			...
			<div class="inventoryBox">
				...
				<div class="profileFrame">
					<img class="profileRank<?=$player->gender?>"
						src="<?php echo base_url(); ?>_images/data/ranks/<?=$player->rank->id?>t.png" height="20"
			 ...
			 if (isset($items) && count($items)) {
                    foreach ($slot_items as $item) {
                        ...
                        <img class="opacityRightHand"   
							...
							title="<?php echo htmlspecialchars($p_item->item->name). ' ' . $p_item->durability . '%'; ?>"
							width="85" />
                        <?php }?>
                        <img
			...
			<div class="squadInfo">
				...
				<div class="squadAtk">
					<b>Squad Attack:</b>
					<div class="squadStatNumbers"><?php echo $squad_attack;?></div>
				</div>
				<div class="squadDef">
					<b>Squad Defense:</b>
					<div class="squadStatNumbers"><?php echo $squad_defense;?></div>
				</div>
                <?php if ($friends) { ?>
                    <ul
...
<?php } ?>
				...
				<div class="large-pad">
					...
					<?php if($vehicle){?>
						<img
							...
							alt="<?php echo htmlspecialchars( $vehicle->item->name); ?>"
							title="<?php echo htmlspecialchars( $vehicle->item->name) . ' ' . $vehicle->durability . '%'; ?>"
							height="88" width="155" />
					...
					<div class="equipStats equipVehicle rarity<?=$vehicle->item->rarity_id?>">
						...
						<img src="<?php echo base_url(); ?>_images/data/items/<?=$vehicle->item->id?>
							...
							alt="<?=htmlspecialchars($vehicle->item->name)?>"
							title="<?=htmlspecialchars($vehicle->item->name) ?>">
						<div class="statBox">
							<table>
                            <?php foreach(Item::$stat_fields as $i => $field) { ?>
            ...
            <div class="companion">
				<div class="large-pad">
					...
					<?php if($companion){ ?>
                        <img src="<?php echo base_url(); ?>_images/data/items/<?=$companion->item->id; ?>.png"
							alt="<?=htmlspecialchars($companion->item->name); ?>"
							title="<?=htmlspecialchars($companion->item->name); ?>"
							height="88" width="155" />
        ...
        <!-- <?php print_r($stats); ?> -->
			<table class="list statDogTagFormat">
				<tr class="statTitle">
					...
					<td><img src="/_images/icons/critical_small.png" alt="critical strike icon"></td>
        ...
        <?php
				...
				<div class="inventoryOptions">
					<form action="<?php echo base_url(); ?>action/equip" method="post">
						<div>
							<input type="hidden" name="player_item_id"
								value="<?php echo $item['player_item_id']; ?>" />
							<input type="submit" class="blue enableItem" value="Enable" />
						</div>
					</form>
