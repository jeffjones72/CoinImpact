...
<div class="box" style="height: 305px;">
            ...
            {% if request.session.level_up %}
                    <div id="popLevel" class="popUp">
                            ...
                            <form id="levelUp" action="<?php echo base_url(); ?>comfirm_levelup" method="post">
                                    <div>
                                            <input id="congratBtn" class="blue center" type="submit" value="OK" onclick="javascript:toggle('popLevel')" />
                                    </div>
                            </form>
                    </div>
            {% endif %}
	...
	<div id="location" class="exploreLocation">
        ...
        <div>
                ...
                <img title="{{c.trader.name}}" src="<?php echo base_url(); ?>_images/data/traders/{{c.trader.id}}t.png" alt=f"" style="float:right;margin: 0 0 10px 10px;" />
                {% endfor %}
                <div class="clear"></div>
        </div>
        -->
	...
	<div class="exploreStatBar">
		<div class="exploreProgressBg">
			...
			<div class="exploreProgressText">
				<strong><?=$player->p_place->place->name?></strong> - <?=$player->p_place->progress?>% explored
				<img class="exploreInfo titleAddBtn"
					src="<?php echo base_url(); ?>_images/icoInfo.png" width="18"
					height="18" alt="Add">
				
				<div class="stat_msg"><p>Complete this zone to advance</p></div>
			</div>
		</div>
		<div id="exploreBar">
...
?>
		...
		<div class="exploreTooltip">
				<img class="exploreBarInfo titleAddBtn" src="<?php echo base_url(); ?>_images/icoInfo.png" width="18" height="18" alt="Add">
				<div class="stat_msg">
					<p>Exploring is your way to discover powerful items, earn coins, gain experience, and complete missions</p>
				</div>
			</div>
<?php
if ($player->energy >= $player->energy && ! isset($p_combatant) && ! isset($p_event) && ! $hasTrader) {
    
    // and not combatant && not boss && not event && not trader %}     ?>
			...
			<div class="fightSectionStart">
                ...
                ?>
					...
					move on.</p>
                                                    <?php } else { ?>
                                                        <p
					class="endBattleText">Click the items you want to pick up.</p>
                                                    <?php } ?>
                ...
                <div id="timeline">
                    <?php foreach ($actions as $action) { ?>
                        <?php if ($action['health'] > 0) { ?>
                            <?=$p_combatant->combatant->name?> hit you for <span class="redText"><?php echo $action['health']; ?> damage</span><br />
                        <?php } else { ?>
                            You dodged <?=$p_combatant->combatant->name?> taking no damage</span><br />
                        <?php } ?>

                        <?php if ($action['damage'] > 0) { ?>
                            You <?php if($action['critical_hit'] == 1) {echo 'critically'; } ?> hit <?=$p_combatant->combatant->name?> for <?php echo $action['damage']; ?> damage<br />
                        <?php } else { ?>
                            <?=$p_combatant->combatant->name?> dodged you taking no damage</span><br />
                        <?php } ?>
                    <?php } ?>
                </div>
