...
class Intro extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->model("Squad_npc_model");
        $data['squad_npc'] = $this->Squad_npc_model->get_NPC_by_intro_part(1);
    ...
    function add_squad_npc()
    {
    $user_id = $this->session->userdata('player_id');

        $intro_part = $this->input->post("intro");
        $this->load->model("Squad_model");
        //add_npc_squad_part($part_id, $player_id)
        if($intro_part != false  ){
            $this->Squad_model->add_npc_squad_part($intro_part, $user_id);
         }
//         if ($intro_part == 1) {
//             $this->Squad_model->add_squad_npc($user_id, 1);
//             $this->Squad_model->add_squad_npc($user_id, 2);
//             // echo $result;
//         }
    }
}
