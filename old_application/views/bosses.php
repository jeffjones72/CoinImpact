...
setInterval(function(){
    if(stat_info['health'] >= 10) {
        $('[data-stamina-btn]').each(function(){
            if(stat_info['stamina'] >= $(this).attr('data-stamina-btn')) {
...
<div class="box" style="height: 850px;">
	...
	<div class="bossLocateBox section" id="locate">
		...
		<form class="bossFilterMenu">
			...
			<div class="bossLocateFilter">
				Sort By: <select>
					<option value="">Cost - acending</option>
					<option value="">Cost - decending</option>
				</select>
			</div>
			<div>
	...
	<div class="bossActiveBox section" id="active">
		<form class="filterMenu">
			<div class="filterCenter">
				Sort By: <select>
					<option value="">Damage (acending)</option>
					<option value="">Damage (decending)</option>
				</select>
			</div>
		</form>
            <?php
...
<?php foreach($combat_log as $info) {?>
                        <?=$info['player_name']?> deals <?=$info['damage_to_boss']?> damage to <?=$current_target->boss->name?>.<br>
        <?=$current_target->boss->name?> deals <?=$info['damage_to_player']?> damage to <?=$info['player_name']?>.<br>
<?php }?>
                    </div>
		</div>
