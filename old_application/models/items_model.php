...
class Items_model extends CI_Model
{
    ...
    function get_items($player_id = 0)
    {
        ...
        if ($query->num_rows > 0) {
            ...
            /*
             * CODE: CI:B0201 calculeted number of items the player has as pieces
             */
            $this->db->select('pi.quality, pi.durability, pi.id as player_item_id, count(i.id) as pieces ,i.id, i.name, i.description, i.price, i.premium_price, i.value, i.section_id,i.has_quality, i.weight, coalesce(pi.mod_atk,0) + i.attack as attack, coalesce(pi.mod_def,0) + i.defense as defense, i.energy, i.stamina, i.health, i.strike, i.strike_boost, i.damage_boost, i.luck, i.dodge, i.capacity, i.modifiers_limit,i.rarity_id', FALSE);
            $this->db->from('player_items pi');
            $this->db->join('items i', 'pi.item_id = i.id');
