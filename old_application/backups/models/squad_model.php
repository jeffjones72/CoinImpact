   ...
   public function get_squad_members_profile($player_id){
    ...
    public function add_squad_npc($player_id, $team_player_id)
    {
        ...
        if ($occupied_slots >= 5) {
            ...
            $where = array("player_id"=>$player_id, "team_player_id"=>$team_player_id, "squad_id != "=>"0");
            $query = $this->db->get_where("players_team", $where);
            ...
            $data = array(
                "player_id" => $player_id,
                "team_player_id" => $team_player_id, "is_NPC" =>"1", "squad_create_date" => $date , "squad_id" =>1
            );
    ...
    /**
     * used to add npc to squad when it appears in $part_id (e.g. part_id = 1 is intro, part_id = 2 in town 2 and so on)
     * @param unknown $part_id part number where NPC appears
    ...
    public function add_npc_squad_part($part_id, $player_id){
        ...
        if ($occupied_slots >= 5) {
            ...
            $ids_array = $this->db->select("id")->from("squad_NPC")->where("level_id = {$part_id} and alive = 1")->get()->result();
