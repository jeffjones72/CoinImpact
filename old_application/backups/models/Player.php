...
class Player extends CI_Model
{
    ...
    public function tryEquip(PlayerItem $p_item)
    {
        ...
        if ($p_item->item->isWeapon()) {
            ...
            if ($leftHandItem != null && $leftHandItem->item->weight == 2) {
                $old_slot_id = Item::LEFT_HAND_SLOT_ID;
            } else {
                $old_slot_id = $slot_id;
            }

            $old_player_item = $p_item->getBySlotId($this->id, $old_slot_id);
            ...
            if ($old_player_item != null) {
                ...
                if ($removeBothHands) { // remove the other item too
                    ...
                    $otherPlayerItem = $p_item->getBySlotId($this->id, $otherSlotId);
                    if ($otherPlayerItem !== null) {
                        $this->unequip($otherPlayerItem);
                    }
                }
                $this->unequipBySlotID($old_slot_id);
            }
            // get the slot for the current weapon

            $slot_id = $this->getEquipSlotForWeapon($p_item, $old_player_item);
        } else {
    ...
    public function unequip(PlayerItem $p_item)
    {
        ...
        $this->db->where('id', $p_item->id);
        $this->db->update('player_items');
        // log_message('debug', json_encode($p_item->item));
        // log_message('debug',"Attack Before:{$this->attack}");
        $this->removeStatsFrom($p_item->item);
        // log_message('debug',"Attack After:{$this->attack}");
        // log_message('debug', json_encode($p_item->item));
        // die('test');
        $this->queryItems();
        return true;
...
WHERE player_id ="' . $this->id . '"');
    ...
    public function attackPlayerBoss(PlayerBoss $p_boss, $stamina)
    {
        ...
        */
        $damageCalculator = new DamageCalculator(array(
            'player' => $this,
            'dodge' => -1 //A boss can't dodge.
        ));
     ...
     */
    public function get_team_players($order_by = null, $limit = null, $start = null, $where_arg = null)
    {
       ...
       // var_dump($this->db->last_query());
        ...
        foreach ($results as $i => $team_player) {
            ...
            // }
            // if ($team_player->team_player_id != $this->id && ! in_array($team_player->team_player_id, $team_players_id)) {
            if($team_player->is_NPC != 1){
        ...
        $this->db->select("players.* , players.id as p_id, players.attack as p_attack, players.defense as p_defense,players.dodge as p_dodge, players.luck as p_luck ,
                ...
                players_team.*, ranks.*, ranks.id as r_id,  (players.attack+ players.defense*0.25)  as power ");

        if(count($team_players_id)){
            //$this->db->where("players.id in ({$id_string})");
            $where_cond = "players.id in ({$id_string}) and";
        }else{
            $where_cond = "";
        }
        if ($where_arg != null) {
            $this->db->where($where_cond." players_team.player_id = {$this->id} and $where_arg");
        } else {
            $this->db->where($where_cond." players_team.player_id = {$this->id}");
        }
        ...
        $query_player = $this->db->get("players");
	if($query->num_rows()>0){
      ...
      //  var_dump($this->db->last_query());die();
        ...
        $this->db->join("players_team", "squad_NPC.id = players_team.team_player_id  and is_NPC = 1 and  players_team.player_id = {$this->id}", "left");
    ...
    public function get_squad_members()
    {
        ...
        $this->db->join("squad_NPC", "players_team.team_player_id = squad_NPC.id", "left");
     ...
     */
    public function get_free_players($order_by = null, $limit = null, $start = null, $where_arg = null)
    {
        ...
        $this->db->join("accounts", "players.account_id = accounts.id", "left");
        $this->db->join("ranks", "players.rank_id = ranks.id", "left");
        $this->db->join("players_team", "players.id = players_team.player_id or players.id=players_team.team_player_id", "left");
        $this->db->group_by("p_id");
        if ($order_by != null) {
            $this->db->order_by($order_by);
        }
